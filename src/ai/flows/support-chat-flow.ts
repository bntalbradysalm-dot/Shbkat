...
1. **كيفية شحن (تغذية) الحساب (حسابات الإيداع)**:
   - يتم الشحن عبر التحويل إلى حساباتنا في: (بنك الأمل، مصرف الكريمي، بنك اليمن والكويت، أو عبر شركات الصرافة المحلية مثل العمقي والبسيري).
   - بعد التحويل، يجب على الزبون الذهاب لصفحة "غذي حسابك"، كتابة المبلغ، ثم الضغط على زر "إرسال الإيصال عبر الواتساب".
   - العملية لا تتم تلقائياً؛ يجب إرسال صورة الإيصال للإدارة عبر الواتساب ليتم التأكد وإضافة الرصيد يدوياً في أسرع وقت.
...
- كن مختصراً ومباشراً.
- إذا لم تكن متأكداً من معلومة تقنية دقيقة، وجه الزبون لمراسلة الإدارة واتساب من صفحة "غذي حسابك".
...
const supportChatFlow = ai.defineFlow(
  ...
  async (input) => {
    try {
        ...
        // في حال فشل الاتصال بالنموذج، نقدم إجابة نصية ثابتة بذكاء بدلاً من رسالة الخطأ
        if (input.message.includes('شحن') || input.message.includes('ايداع')) {
            return { text: "لشحن حسابك، يرجى التحويل لأحد حساباتنا (الكريمي أو الأمل) ثم إرسال صورة الإيصال عبر صفحة 'غذي حسابك' بالواتساب للإدارة." };
        }
        return { text: "أهلاً بك، أنا هنا لمساعدتك. بخصوص استفسارك، يفضل التواصل مع الدعم المباشر عبر الواتساب من صفحة 'غذي حسابك' لخدمتك بشكل أسرع حالياً." };
    }
  }
);