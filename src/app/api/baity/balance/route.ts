...
const API_BASE_URL = 'https://apis.okamel.org/api/partner-yem/check-balance';
...
export async function POST(request: Request) {
  try {
    ...
    if (!mobile) {
      return new NextResponse(JSON.stringify({ message: 'Mobile number is required' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
    }
    ...
    console.error('Error in /api/baity/balance route:', error);
    return new NextResponse(
