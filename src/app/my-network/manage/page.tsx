...
export default function OwnerNetworkManagePage() {
  ...
  const networkQuery = useMemoFirebase(() => (
    user ? query(collection(firestore, 'networks'), where('ownerId', '==', user.uid)) : null
  ), [user, firestore]);
     ...
     return (
        <div className="flex flex-col h-full bg-background">
            ...
            <div className="flex-1 flex items-center justify-center p-4">
               <p className='text-center text-muted-foreground'>لم يتم العثور على شبكة مرتبطة بحسابك.</p>
            </div>
      </div>
    )
  }
    ...
    <>
      <div className="flex flex-col h-full bg-background">
        <SimpleHeader title={`إدارة: ${network.name}`} />
         <p className="text-center text-xs text-muted-foreground -mt-2 mb-4 px-4">
