...
function NetworkPurchasePageComponent() {
  ...
  const handlePurchase = async () => {
    if (!selectedCategory || !user || !userProfile || !firestore || !userDocRef || !networkData) return;
    ...
    try {
        ...
        if (availableCardsSnapshot.empty) {
            throw new Error(`عفواً، لا توجد كروت متاحة في فئة "${selectedCategory.name}" حالياً.`);
        }
        ...
        // 2. Create a record in soldCards
        ...
        // 4. Add earnings to network owner's balance
        ...
        toast({ variant: "destructive", title: "فشلت عملية الشراء", description: error.message || "حدث خطأ غير متوقع. الرجاء المحاولة مرة أخرى." });
    } finally {
        setIsProcessing(false);
        setIsConfirming(false);
    }
  ...
  const handleCopyCardDetails = () => {
    if (purchasedCard) {
        ...
        toast({ title: "تم النسخ", description: "تم نسخ تفاصيل الكرت بنجاح." });
    }
  };
                ...
                <CardContent className="p-6">
                    <div className="flex flex-col items-center justify-center gap-4">
                        ...
                        <div className="w-full text-right space-y-2 bg-muted p-3 rounded-lg mt-2 font-mono">
                           <p>Card: {purchasedCard.cardNumber}</p>
                           <p className='text-xs'>(لا توجد كلمة مرور لهذه الكروت)</p>
                        </div>
