...
import { doc, writeBatch, increment, collection, query, where, limit, getDocs } from 'firebase/firestore';
...
function NetworkCardsPageComponent() {
  ...
  const categoriesCollectionRef = useMemoFirebase(() => firestore ? collection(firestore, `networks/${networkId}/cardCategories`) : null, [firestore, networkId]);
  const { data: categories, isLoading: isLoadingCategories, error } = useCollection<CardCategory>(categoriesCollectionRef);
  ...
  const buyerDocRef = useMemo(() => (user && firestore ? doc(firestore, 'users', user.uid) : null), [user, firestore]);
  const { data: buyerProfile } = useDoc<UserProfile>(buyerDocRef);
  
  const handlePurchase = async () => {
    if (!selectedCategory || !user || !buyerProfile || !firestore || !buyerDocRef || !networkData || !networkData.ownerId) return;

    setIsProcessing(true);
    const categoryPrice = selectedCategory.price;
    const userBalance = buyerProfile?.balance ?? 0;
    ...
    try {
        const cardsCollectionRef = collection(firestore, `networks/${networkId}/cards`);
        const q = query(cardsCollectionRef, where('categoryId', '==', selectedCategory.id), where('status', '==', 'available'), limit(1));
        const availableCardsSnapshot = await getDocs(q);

        if (availableCardsSnapshot.empty) {
            throw new Error('لا توجد كروت متاحة في هذه الفئة حاليًا.');
        }

        const cardToPurchaseDoc = availableCardsSnapshot.docs[0];
        const cardToPurchaseData = cardToPurchaseDoc.data() as NetworkCard;
        const cardId = cardToPurchaseDoc.id;
        ...
        // 1. Update the purchased card
        const cardRef = doc(firestore, `networks/${networkId}/cards`, cardId);
        batch.update(cardRef, { status: 'sold', soldTo: user.uid, soldTimestamp: now });

        // 2. Create a record in soldCards collection
        ...
        batch.set(soldCardRef, {
            ...
            buyerId: user.uid,
            buyerName: buyerProfile.displayName || 'غير معروف',
            buyerPhoneNumber: buyerProfile.phoneNumber || 'غير معروف',
            soldTimestamp: now,
            payoutStatus: 'pending'
        });

        // 3. Deduct balance from buyer
        batch.update(buyerDocRef, { balance: increment(-categoryPrice) });
        
        // 4. Create a transaction record for buyer
        const buyerTransactionRef = doc(collection(firestore, `users/${user.uid}/transactions`));
        batch.set(buyerTransactionRef, {
            userId: user.uid, transactionDate: now, amount: categoryPrice, transactionType: `شراء كرت ${selectedCategory.name}`,
            notes: `شبكة: ${networkName}`, cardNumber: cardToPurchaseData.cardNumber, cardPassword: 'N/A'
        });

        // 5. Add earnings to network owner's balance
        const ownerDocRef = doc(firestore, 'users', networkData.ownerId);
        batch.update(ownerDocRef, { balance: increment(payoutAmount) });

        // 6. Create a transaction record for owner
        const ownerTransactionRef = doc(collection(firestore, `users/${networkData.ownerId}/transactions`));
        batch.set(ownerTransactionRef, {
            userId: networkData.ownerId, transactionDate: now, amount: payoutAmount,
            transactionType: 'أرباح بيع كرت', notes: `بيع كرت ${selectedCategory.name} للمستخدم ${buyerProfile.displayName}`
        });
        
        await batch.commit();

        setPurchasedCard({ cardID: cardToPurchaseData.cardNumber, cardPass: 'N/A' });
...
export default function NetworkCardsPage() {
    return (
      <Suspense fallback={<div className="flex items-center justify-center h-full"><Skeleton className="h-32 w-full" /></div>}>
