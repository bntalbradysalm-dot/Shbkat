...
// --- STYLES ---
const WHY_PRIMARY = '#713CA4'; 
const WHY_GRADIENT = {
    backgroundColor: '#713CA4',
    backgroundImage: `
        radial-gradient(at 0% 0%, #8E54C1 0px, transparent 50%),
        radial-gradient(at 100% 100%, #5A2D85 0px, transparent 50%)
    `
};
...
const PackageItemCard = ({ offer, onClick }: { offer: Offer, onClick: () => void }) => (
    <div 
      className="bg-white dark:bg-slate-900 rounded-3xl p-5 shadow-sm relative border border-[#713CA4]/10 mb-3 text-center cursor-pointer hover:bg-[#713CA4]/5 transition-all active:scale-[0.98] group"
      onClick={onClick}
    >
      ...
      <div className="grid grid-cols-4 gap-2 pt-3 mt-2 border-t border-[#713CA4]/10 text-center">
        <div className="space-y-1.5">
            <Globe className="w-5 h-5 mx-auto text-[#713CA4]" />
            <p className="text-[11px] font-black text-foreground truncate">{offer.data || '-'}</p>
        </div>
        <div className="space-y-1.5">
            <Mail className="w-5 h-5 mx-auto text-[#713CA4]" />
            <p className="text-[11px] font-black text-foreground truncate">{offer.sms || '-'}</p>
        </div>
        <div className="space-y-1.5">
            <PhoneIcon className="w-5 h-5 mx-auto text-[#713CA4]" />
            <p className="text-[11px] font-black text-foreground truncate">{offer.minutes || '-'}</p>
        </div>
        <div className="space-y-1.5">
            <Clock className="w-5 h-5 mx-auto text-[#713CA4]" />
...
const FastOfferCard = ({ offer, onClick }: { offer: FastOffer, onClick: () => void }) => (
    <div 
      className="bg-white dark:bg-slate-900 rounded-3xl p-4 shadow-sm border border-[#713CA4]/10 mb-3 cursor-pointer hover:bg-[#713CA4]/5 transition-all active:scale-[0.98] group flex items-center justify-between"
      onClick={onClick}
    >
      <div className="flex items-center gap-4 text-right">
          <div className="relative h-12 w-12 overflow-hidden rounded-2xl border border-[#713CA4]/20 bg-white shrink-0">
              <Image src="https://i.postimg.cc/SN1vL3BC/images-(1).jpg" alt="WHY" fill className="object-cover" />
          </div>
          <div className="flex flex-col items-start">
              <h4 className="text-sm font-black text-foreground group-hover:text-[#713CA4] transition-colors">{offer.title}</h4>
      ...
      <div className="flex items-baseline text-left shrink-0">
        <span className="text-xl font-black text-[#713CA4]">{offer.price.toLocaleString('en-US')}</span>
      </div>
    </div>
);
    ...
    return (
        <div className="flex flex-col h-full bg-[#FDFDFD] dark:bg-slate-950">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                <div className="bg-white dark:bg-slate-900 rounded-3xl p-4 shadow-sm border border-[#713CA4]/5">
                    ...
                    <div className="relative">
                        <Input
                            ...
                            className="text-center font-bold text-lg h-12 rounded-2xl border-none bg-muted/20 focus-visible:ring-[#713CA4] transition-all pr-12 pl-12"
                        />
                        <button 
                            onClick={handleContactPick}
                            className="absolute left-3 top-1/2 -translate-y-1/2 p-2 text-[#713CA4] hover:bg-[#713CA4]/10 rounded-xl transition-colors"
                            title="جهات الاتصال"
                        >
                ...
                {phone.length === 9 && (
                    <div className="space-y-4 animate-in fade-in-0 slide-in-from-bottom-4 duration-500">
                        <Tabs defaultValue="packages" value={activeTab} onValueChange={setActiveTab} className="w-full">
                            <TabsList className="grid w-full grid-cols-2 bg-white dark:bg-slate-900 rounded-2xl h-14 p-1.5 shadow-sm border border-[#713CA4]/5">
                                <TabsTrigger value="packages" className="rounded-xl font-bold text-xs data-[state=active]:bg-[#713CA4] data-[state=active]:text-white">الباقات</TabsTrigger>
                                <TabsTrigger value="balance" className="rounded-xl font-bold text-xs data-[state=active]:bg-[#713CA4] data-[state=active]:text-white">رصيد</TabsTrigger>
                            </TabsList>
                                        ...
                                        <AccordionItem key={cat.id} value={cat.id} className="border-none">
                                            <AccordionTrigger className="px-4 py-4 rounded-2xl text-white hover:no-underline shadow-md group data-[state=open]:rounded-b-none" style={WHY_GRADIENT}>
                                                <div className="flex items-center gap-3 flex-1">
                                                    <div className="bg-white text-[#713CA4] font-black text-xs px-3 py-1 rounded-xl shadow-inner shrink-0">{cat.badge}</div>
                            ...
                            <TabsContent value="balance" className="pt-2 animate-in fade-in-0 duration-300">
                                <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-sm border border-[#713CA4]/5 text-center">
                                    ...
                                    <div className="relative max-w-[240px] mx-auto">
                                        <Input 
                                            ...
                                            className="text-center font-black text-3xl h-16 rounded-2xl bg-muted/20 border-none text-[#713CA4] placeholder:text-[#713CA4]/10 focus-visible:ring-[#713CA4]" 
                                        />
                                        <div className="absolute left-4 top-1/2 -translate-y-1/2 text-[#713CA4]/30 font-black text-sm">ر.ي</div>
                                    </div>
                                    <Button 
                                        className="w-full h-14 rounded-2xl text-lg font-black mt-8 shadow-lg shadow-[#713CA4]/20 text-white" 
                ...
                <AlertDialogContent className="rounded-[32px]">
                    <AlertDialogHeader>
                        ...
                        <div className="space-y-3 pt-4 text-right text-sm">
                            ...
                            <div className="flex justify-between items-center py-3 bg-muted/50 rounded-xl px-2 mt-2"><span className="font-black">إجمالي الخصم:</span><span className="font-black text-[#713CA4] text-lg">{(parseFloat(amount || '0') + Math.ceil(parseFloat(amount || '0') * 0.05)).toLocaleString('en-US')} ريال</span></div>
                        </div>
                    </AlertDialogHeader>
                ...
                <AlertDialogContent className="rounded-[32px]">
                    <AlertDialogHeader>
                        ...
                        <div className="py-4 space-y-3 text-right text-sm">
                            <p className="text-center text-lg font-black text-[#713CA4] mb-2">{selectedFastOffer?.title}</p>
                            ...
                            <div className="flex justify-between items-center py-3 bg-muted/50 rounded-xl px-2 mt-2"><span className="font-black">إجمالي الخصم:</span><span className="font-black text-[#713CA4] text-lg">{((selectedFastOffer?.price || 0) + Math.ceil((selectedFastOffer?.price || 0) * 0.05)).toLocaleString('en-US')} ريال</span></div>
                        </div>
                    </AlertDialogHeader>
                    <AlertDialogFooter className="grid grid-cols-2 gap-3 mt-6 sm:space-x-0">
            ...
            <AlertDialog open={!!selectedOffer} onOpenChange={() => setSelectedOffer(null)}>
                <AlertDialogContent className="rounded-[32px]">
                    <AlertDialogHeader>
                        ...
                        <div className="py-4 space-y-3 text-right text-sm">
                            <p className="text-center text-lg font-black text-[#713CA4] mb-2">{selectedOffer?.offerName}</p>
                            <div className="flex justify-between items-center py-2 border-b border-dashed"><span className="text-muted-foreground">رقم الجوال:</span><span className="font-bold">{phone}</span></div>
                            ...
                            <div className="flex justify-between items-center py-3 bg-muted/50 rounded-xl px-2 mt-2"><span className="font-black">إجمالي الخصم:</span><span className="font-black text-[#713CA4] text-lg">{((selectedOffer?.price || 0) + Math.ceil((selectedOffer?.price || 0) * 0.05)).toLocaleString('en-US')} ريال</span></div>
                        </div>
                    </AlertDialogHeader>
                    <AlertDialogFooter className="grid grid-cols-2 gap-3 mt-6 sm:space-x-0">
                        <AlertDialogCancel className="w-full rounded-2xl h-12 mt-0">تراجع</AlertDialogCancel>
                ...
                <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center animate-in fade-in-0 p-4">
                    ...
                    <Card className="w-full max-w-sm text-center shadow-2xl rounded-[40px] overflow-hidden border-none bg-card">
                        ...
                        <CardContent className="p-8 space-y-6">
                            ...
                            <div className="w-full space-y-3 text-sm bg-muted/50 p-5 rounded-[24px] text-right border-2 border-dashed border-[#713CA4]/10">
                                <div className="flex justify-between items-center border-b border-muted pb-2"><span className="text-muted-foreground flex items-center gap-2"><Hash className="w-3.5 h-3.5" /> رقم العملية:</span><span className="font-mono font-black text-[#713CA4]">{lastTxDetails.transid}</span></div>
                                ...
                                <div className="flex justify-between items-center border-b border-muted pb-2"><span className="text-muted-foreground flex items-center gap-2"><Wallet className="w-3.5 h-3.5" /> المبلغ:</span><span className="font-black text-[#713CA4]">{lastTxDetails.amount.toLocaleString('en-US')} ريال</span></div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <Button className="rounded-2xl h-12 font-bold text-white" onClick={() => { setShowSuccess(false); }} style={{ backgroundColor: WHY_PRIMARY }}>إغلاق</Button>
                            </div>
                        </CardContent>
                    </Card>
                </div>
