    ...
    >
      ...
      <div className="grid grid-cols-4 gap-0 pt-2 border-t border-[#EBCDB5] text-center">
        <div className="space-y-1">
          ...
          <p className="text-[11px] font-black text-slate-800">{offer.data || '-'}</p>
        </div>
        <div className="space-y-1 border-r border-[#EBCDB5]">
          <Mail className="w-5 h-5 mx-auto text-[#8B1D3D]" />
          <p className="text-[11px] font-black text-slate-800">{offer.sms || '-'}</p>
        </div>
        <div className="space-y-1 border-r border-[#EBCDB5]">
          <Phone className="w-5 h-5 mx-auto text-[#8B1D3D]" />
          <p className="text-[11px] font-black text-slate-800">{offer.minutes || '-'}</p>
        </div>
        <div className="space-y-1 border-r border-[#EBCDB5]">
          <Clock className="w-5 h-5 mx-auto text-[#8B1D3D]" />
          <p className="text-[11px] font-black text-slate-800">{offer.validity || '-'}</p>
        </div>
      </div>
    </div>
  );
                                ...
                                <Accordion type="single" collapsible className="w-full space-y-3">
                                  {CATEGORIES.map((category) => (
                                    <AccordionItem key={category.id} value={category.id} className="border-none">
                                      <AccordionTrigger className="px-4 py-2 text-sm hover:no-underline bg-primary rounded-xl shadow-md transition-all active:scale-[0.98] group flex-row-reverse [&[data-state=open]]:rounded-b-none">
                                        <div className="flex items-center gap-3 flex-1">
                                            ...
                                            <span className="text-white font-black text-right flex-1 text-xs tracking-wide">{category.title}</span>
                                        </div>
                                        <ChevronDown className="w-3.5 h-3.5 text-white/80 transition-transform group-data-[state=open]:rotate-180 ml-auto" />
                                      </AccordionTrigger>
                                      <AccordionContent className="p-3 space-y-2 bg-white border-x border-b rounded-b-xl shadow-inner">
