    ...
    <header className="flex items-center justify-between p-4 bg-transparent text-foreground relative h-24">
      <div className="flex items-center gap-3 flex-1 px-2">
        ...
        <div className="flex flex-col items-start justify-center">
          {isUserLoading ? (
            <div className="space-y-2 text-right"><Skeleton className="h-3 w-16" /><Skeleton className="h-5 w-28" /></div>
          ) : (
            <><p className="text-primary font-black text-sm opacity-90 leading-tight">{greeting}</p><h1 className="font-black text-foreground text-base tracking-tight leading-tight">{displayName}</h1></>
          )}
        </div>
      </div>
      <button onClick={handleNotificationClick} className="relative p-2.5 bg-muted/20 rounded-full border border-border/50">
        <Bell className="h-6 w-6 text-primary" />
        {unreadCount > 0 && <span className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-destructive text-white text-[10px] font-bold animate-in zoom-in">{unreadCount > 9 ? '+9' : unreadCount}</span>}
      </button>
    </header>
  );
};
